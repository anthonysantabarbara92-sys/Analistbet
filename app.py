        ...
        if cerca and h_name and a_name:
            ...
            with st.spinner(f'Stats avanzate {h_name}...'):
                ...
                if hs: banners.append(('ok',f'✅ {h_name}: {hs.matches_n} partite | GF: {hs.gf_avg:.2f} | xG: {hs.xg_avg:.2f} | Casa: {hs.gf_home:.2f} | Trasf: {hs.gf_away:.2f}')); raw['h_stats']=hs
                    ...
                    if claude_key:
                        bk=claude_backup(claude_key,h_name,a_name,comp,'statistiche')
                        if bk:
                            raw['h_stats']=TeamStats(name=h_name,gf_avg=bk.get('gf_home',1.3),ga_avg=bk.get('ga_home',1.1),source='Claude AI')
                            banners.append(('ok',f'✅ {h_name} da Claude AI (backup)'))
            ...
            with st.spinner(f'Stats avanzate {a_name}...'):
                ...
                if as_: banners.append(('ok',f'✅ {a_name}: {as_.matches_n} partite | GF: {as_.gf_avg:.2f} | xG: {as_.xg_avg:.2f} | Casa: {as_.gf_home:.2f} | Trasf: {as_.gf_away:.2f}')); raw['a_stats']=as_
                    ...
                    if claude_key:
                        bk=claude_backup(claude_key,h_name,a_name,comp,'statistiche')
                        if bk:
                            raw['a_stats']=TeamStats(name=a_name,gf_avg=bk.get('gf_away',1.1),ga_avg=bk.get('ga_away',1.2),source='Claude AI')
                            banners.append(('ok',f'✅ {a_name} da Claude AI (backup)'))
                    ...
                    if odds_data:
                        ...
                        if claude_key:
                            with st.spinner('Claude cerca le quote...'):
                                bk=claude_backup(claude_key,h_name,a_name,comp,'quote bookmaker')
                                if bk: raw['odds']=bk; banners.append(('ok','✅ Quote da Claude AI (backup)'))
            else:
