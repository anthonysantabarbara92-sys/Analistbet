...
try:
    from engine import (
        ...
        LEAGUE_AVGS, fetch_af_stats, fetch_understat_xg,
        fetch_clubelo, elo_lambda_adjustment,
    )
    ENGINE_OK = True
...
def fetch_odds(odds_key, sport_key, home, away):
    try:
        p={'apiKey':odds_key,'regions':'eu','markets':'h2h,totals,btts',
           'oddsFormat':'decimal'}
        # Try specific sport first, then fallback to soccer_epl as test
        r=requests.get(f'https://api.the-odds-api.com/v4/sports/{sport_key}/odds',
                       params=p,headers=REQ_H,timeout=12)
        if r.status_code==422 or r.status_code==404:
            # Fallback: search across all soccer sports
            r2=requests.get('https://api.the-odds-api.com/v4/sports',
                           params={'apiKey':odds_key},headers=REQ_H,timeout=8)
            if r2.status_code==200:
                soccer_sports=[s['key'] for s in r2.json() if 'soccer' in s.get('key','') and s.get('active')]
                for sk in soccer_sports[:5]:
                    r=requests.get(f'https://api.the-odds-api.com/v4/sports/{sk}/odds',
                                   params=p,headers=REQ_H,timeout=8)
        ...
        hl,al=home.lower(),away.lower()
        def _norm(s):
            for x in [' fc',' cf',' ac',' sc',' afc',' fk',' calcio',' united',' city',' asd',' ssd',' ssc',' ss',' us ']:
                s=s.replace(x,'')
            return s.strip()
        hln,aln=_norm(hl),_norm(al)
        event=None
        for strict in [True,False]:
            for ev in data:
                eh=_norm(ev.get('home_team','').lower())
                ea=_norm(ev.get('away_team','').lower())
                if strict:
                    if (hln==eh or hln in eh or eh in hln) and (aln==ea or aln in ea or ea in aln):
                        event=ev; break
                else:
                    hm=any(t in eh for t in hln.split() if len(t)>3)
                    am=any(t in ea for t in aln.split() if len(t)>3)
                    if hm and am: event=ev; break
            if event: break
...
def render_combo(c:ComboResult):
    try:
        ...
        legs_parts = []
        for l in c.legs:
            v = float(l.bk) if float(l.bk)>1.01 else float(l.fair)
            legs_parts.append(
                '<div class="combo-leg">'
                '<span>' + str(l.emoji) + ' ' + str(l.name) + '</span>'
                '<span style="color:var(--yel)">' + '{:.2f}'.format(v) + '</span>'
                '</div>'
            )
        legs_html = ''.join(legs_parts)
        prob_str  = '{:.1f}%'.format(float(c.prob_combo)*100)
        quota_str = '{:.2f}'.format(float(c.quota_combo))
        st.markdown(
            '<div class="combo-card"><div class="combo-title">' + str(c.tipo) + '</div>'
            + legs_html +
            '<div class="combo-footer">'
            '<span style="font-size:.7rem;color:var(--t2)">Prob: ' + prob_str + '</span>'
            '<span style="font-size:.8rem;font-weight:700;color:' + ec + '">Quota: ' + quota_str + '</span>'
            '</div></div>',
            unsafe_allow_html=True)
    except Exception as e:
        st.markdown(f'<div class="combo-card"><div class="combo-title">{c.tipo}</div>'
                    f'<div style="font-size:.7rem;color:var(--red)">Errore render: {e}</div></div>',
                    unsafe_allow_html=True)
...
# ==============================================================================
def main():
    st.set_page_config(page_title='Quantum Football v8.0.0',page_icon='QF',
    ...
    with st.sidebar:
        ...
        st.markdown(f'<div style="font-size:.68rem;color:#ff6b35">Soglia attiva: {edge_min}%</div>',
                    unsafe_allow_html=True)
        st.divider()
        st.markdown('<div style="font-size:.62rem;color:#555">v8.0 FINAL — Gemini AI | Monte Carlo | Stanchezza | Arbitro</div>',unsafe_allow_html=True)
    fd_key = FD_KEY
    odds_key = ODDS_KEY

    st.markdown(
        '<div class="header"><h1>⚽ Quantum Football v8.0.0 FINAL</h1>'
    ...
    if st.session_state['step']=='input':
        ...
        if cerca and h_name and a_name:
            # Pulisce i widget dell'analisi precedente
            for k in ['q_home', 'q_draw', 'q_away', 'q_ov25', 'q_un25', 'q_ov35', 'q_un35', 'q_btts', 'q_bttsn', 'h_gf', 'h_ga', 'h_xg', 'h_xga', 'h_cor', 'h_crd', 'h_sot', 'h_form', 'a_gf', 'a_ga', 'a_xg', 'a_xga', 'a_cor', 'a_crd', 'a_sot', 'a_form', 'sp_home', 'sp_draw', 'sp_away', 'sp_ov25', 'sp_btts', 'motiv_h', 'motiv_a', 'days_h', 'days_a', 'pressure_h', 'pressure_a', 'pres_h', 'pres_a', 'ref_name', 'ref_style']: st.session_state.pop(k,None)
            ...
            # Understat xG reale
            with st.spinner('Understat: xG reale...'):
                try:
                    ust = fetch_understat_xg(h_name, a_name, comp)
                    if ust:
                        if raw.get('h_stats'): raw['h_stats'].xg_avg=ust.get('h_xg',raw['h_stats'].xg_avg); raw['h_stats'].xga_avg=ust.get('h_xga',raw['h_stats'].xga_avg)
                        if raw.get('a_stats'): raw['a_stats'].xg_avg=ust.get('a_xg',raw['a_stats'].xg_avg); raw['a_stats'].xga_avg=ust.get('a_xga',raw['a_stats'].xga_avg)
                        banners.append(('ok',f'✅ xG reale Understat: {h_name}={ust.get("h_xg","?")} | {a_name}={ust.get("a_xg","?")}'))
                    else:
                        banners.append(('warn','⚠️ Understat: xG non disponibile per questa lega'))
                except Exception as e:
                    banners.append(('warn',f'⚠️ Understat errore: {e}'))

            # API-Football statistiche reali
            with st.spinner('API-Football: statistiche...'):
                try:
                    af = fetch_af_stats(h_name, a_name, comp)
                    if af:
                        if raw.get('h_stats'):
                            if af.get('h_shots_avg'): raw['h_stats'].shots_ot_avg=af['h_shots_avg']
                            if af.get('h_corners_avg'): raw['h_stats'].corners_avg=af['h_corners_avg']
                            if af.get('h_cards_avg'): raw['h_stats'].cards_avg=af['h_cards_avg']
                        if raw.get('a_stats'):
                            if af.get('a_shots_avg'): raw['a_stats'].shots_ot_avg=af['a_shots_avg']
                            if af.get('a_corners_avg'): raw['a_stats'].corners_avg=af['a_corners_avg']
                            if af.get('a_cards_avg'): raw['a_stats'].cards_avg=af['a_cards_avg']
                        banners.append(('ok','✅ Statistiche reali da API-Football'))
                except Exception as e:
                    banners.append(('warn',f'⚠️ API-Football errore: {e}'))

            # ClubElo forza relativa
            with st.spinner('ClubElo: forza relativa...'):
                try:
                    elo_h = fetch_clubelo(h_name)
                    elo_a = fetch_clubelo(a_name)
                    raw['elo_h']=elo_h; raw['elo_a']=elo_a
                    if elo_h>0 and elo_a>0:
                        banners.append(('ok',f'✅ ClubElo: {h_name}={elo_h:.0f} | {a_name}={elo_a:.0f}'))
                except Exception as e:
                    banners.append(('warn',f'⚠️ ClubElo errore: {e}'))
    ...
    # ======================================================
        ...
        with pc1: pressure_h=st.slider(f'{h_name} pressione',-10,10,0,1,key='pressure_h')
        with pc2: pressure_a=st.slider(f'{a_name} pressione',-10,10,0,1,key='pressure_a')
        ...
        with ref2:
            ...
            ref_mult={'Molto permissivo':0.7,'Permissivo':0.85,'Normale':1.0,'Rigido':1.25,'Molto rigido':1.5}[ref_style]
        ...
        with st.expander('💰 Quote — modifica se necessario',expanded=True):
            ...
            st.markdown('<div style="font-size:.65rem;color:var(--t2);margin:10px 0 6px">🎯 SPORTIUM (opzionale — lascia 0 per ignorare)</div>',unsafe_allow_html=True)
            sp1,sp2,sp3=st.columns(3)
            with sp1: sp_home=st.number_input('Sportium 1',0.0,50.0,0.0,0.05,key='sp_home')
            with sp2: sp_draw=st.number_input('Sportium X',0.0,50.0,0.0,0.05,key='sp_draw')
            with sp3: sp_away=st.number_input('Sportium 2',0.0,50.0,0.0,0.05,key='sp_away')
            sp4,sp5=st.columns(2)
            with sp4: sp_ov25=st.number_input('Sportium O2.5',0.0,50.0,0.0,0.05,key='sp_ov25')
            with sp5: sp_btts=st.number_input('Sportium BTTS Si',0.0,50.0,0.0,0.05,key='sp_btts')

        cb1,cb2=st.columns(2)
        with cb1: analizza=st.button('⚡ ANALIZZA',use_container_width=True)
        with cb2:
            if st.button('🔄 Reset', use_container_width=True, key='reset_rev'):
                for k in ['step','raw','banners','result_v6','h_name','a_name']+['q_home', 'q_draw', 'q_away', 'q_ov25', 'q_un25', 'q_ov35', 'q_un35', 'q_btts', 'q_bttsn', 'h_gf', 'h_ga', 'h_xg', 'h_xga', 'h_cor', 'h_crd', 'h_sot', 'h_form', 'a_gf', 'a_ga', 'a_xg', 'a_xga', 'a_cor', 'a_crd', 'a_sot', 'a_form', 'sp_home', 'sp_draw', 'sp_away', 'sp_ov25', 'sp_btts', 'motiv_h', 'motiv_a', 'days_h', 'days_a', 'pressure_h', 'pressure_a', 'pres_h', 'pres_a', 'ref_name', 'ref_style']:
                    st.session_state.pop(k,None)
                st.session_state['step']='input'; st.rerun()

        if analizza:
            h=TeamStats(name=h_name,gf_avg=st.session_state.get('h_gf',h_raw.gf_avg),
                        ga_avg=st.session_state.get('h_ga',h_raw.ga_avg),
                        xg_avg=st.session_state.get('h_xg',h_raw.xg_avg),
                        xga_avg=st.session_state.get('h_xga',h_raw.xga_avg),
                        corners_avg=st.session_state.get('h_cor',h_raw.corners_avg),
                        cards_avg=st.session_state.get('h_crd',h_raw.cards_avg),
                        shots_ot_avg=st.session_state.get('h_sot',h_raw.shots_ot_avg),
                        form=st.session_state.get('h_form',h_raw.form).upper(),
            ...
            a=TeamStats(name=a_name,gf_avg=st.session_state.get('a_gf',a_raw.gf_avg),
                        ga_avg=st.session_state.get('a_ga',a_raw.ga_avg),
                        xg_avg=st.session_state.get('a_xg',a_raw.xg_avg),
                        xga_avg=st.session_state.get('a_xga',a_raw.xga_avg),
                        corners_avg=st.session_state.get('a_cor',a_raw.corners_avg),
                        cards_avg=st.session_state.get('a_crd',a_raw.cards_avg),
                        shots_ot_avg=st.session_state.get('a_sot',a_raw.shots_ot_avg),
                        form=st.session_state.get('a_form',a_raw.form).upper(),
            ...
            # Apply ClubElo strength adjustment
            elo_h_v=float(raw.get('elo_h',0.0)); elo_a_v=float(raw.get('elo_a',0.0))
            elo_mh,elo_ma = elo_lambda_adjustment(elo_h_v, elo_a_v)
            if elo_h_v>0:
                h.gf_avg=round(h.gf_avg*elo_mh,3)
                a.gf_avg=round(a.gf_avg*elo_ma,3)
            q_home=st.session_state.get('q_home',0.0)
            q_draw=st.session_state.get('q_draw',0.0)
            q_away=st.session_state.get('q_away',0.0)
            q_ov25=st.session_state.get('q_ov25',0.0)
            q_un25=st.session_state.get('q_un25',0.0)
            q_ov35=st.session_state.get('q_ov35',0.0)
            q_un35=st.session_state.get('q_un35',0.0)
            q_btts=st.session_state.get('q_btts',0.0)
            q_bttsn=st.session_state.get('q_bttsn',0.0)
            motiv_h=st.session_state.get('motiv_h',0)
            motiv_a=st.session_state.get('motiv_a',0)
            days_h=st.session_state.get('days_h',7)
            days_a=st.session_state.get('days_a',7)
            pressure_h=st.session_state.get('pressure_h',0)
            pressure_a=st.session_state.get('pressure_a',0)
            ref_style=st.session_state.get('ref_style','Normale')
            ref_mult_map={'Molto permissivo':0.7,'Permissivo':0.85,'Normale':1.0,'Rigido':1.25,'Molto rigido':1.5}
            ref_mult=ref_mult_map.get(ref_style,1.0)
            sp_home=st.session_state.get('sp_home',0.0)
            sp_draw=st.session_state.get('sp_draw',0.0)
            sp_away=st.session_state.get('sp_away',0.0)
            sp_ov25=st.session_state.get('sp_ov25',0.0)
            sp_btts=st.session_state.get('sp_btts',0.0)
            ...
            sportium={'home':sp_home,'draw':sp_draw,'away':sp_away,'over25':sp_ov25,'btts_y':sp_btts}
            inj_h_s=raw.get('inj_h',[]); inj_a_s=raw.get('inj_a',[])
            imp_h=estimate_injury_impact(inj_h_s,h_name)
            imp_a=estimate_injury_impact(inj_a_s,a_name)
            eng=AdvancedEngine(h,a,motiv_h,motiv_a,inj_h_s,inj_a_s,h2h_recs,
            ...
            conf=confidence_score(h,a,h2h_recs,inj_h_s,inj_a_s,odds)
            ...
            result=dict(h=h,a=a,comp=comp,eng=eng,mkts=mkts,combos=combos,
                        ...
                        inj_h=inj_h_s,inj_a=inj_a_s,h2h_recs=h2h_recs,h2h_sum=h2h_sum,
            ...
            hist.insert(0,{'match':f'{h_name} vs {a_name}','comp':comp,
                           ...
                           'lh':eng.lh,'la':eng.la,'diff':cmp.get('diff_pct',0)})
    ...
    # ======================================================
        ...
        # SPORTIUM VALUE CHECK
        sp_keys = [('home','1 Casa'),('draw','X Pari'),('away','2 Trasf'),('over25','Over 2.5'),('btts_y','BTTS Si')]
        sp_active = [(k,lbl) for k,lbl in sp_keys if sportium.get(k,0)>1.01]
        if sp_active:
            ph,pd,pa = eng.p1x2()
            ov25,_ = eng.pou(2.5)
            btts,_ = eng.pbtts()
            probs = {'home':ph,'draw':pd,'away':pa,'over25':ov25,'btts_y':btts}
            sp_html = '<div style="background:rgba(255,107,53,.07);border:1px solid rgba(255,107,53,.3);border-radius:10px;padding:12px 16px;margin-bottom:12px">'
            sp_html += '<div style="font-size:.6rem;color:var(--org);font-weight:700;letter-spacing:.12em;text-transform:uppercase;margin-bottom:10px">🎯 Sportium Value Check</div>'
            for k,lbl in sp_active:
                sq = float(sportium[k]); prob = probs.get(k,0)
                implied = 1/sq if sq>1.01 else 0
                edge_sp = round((prob*sq-1)*100,1)
                col = 'var(--grn)' if edge_sp>5 else ('var(--yel)' if edge_sp>0 else 'var(--red)')
                sem = '🟢' if edge_sp>5 else ('🟡' if edge_sp>0 else '🔴')
                sp_html += (
                    '<div style="display:flex;justify-content:space-between;align-items:center;'
                    'padding:7px 0;border-bottom:1px solid rgba(255,255,255,.05)">'
                    '<span style="font-size:.73rem">' + sem + ' ' + lbl + '</span>'
                    '<span style="font-size:.65rem;color:var(--t2)">Quota: ' + '{:.2f}'.format(sq) +
                    ' | Modello: ' + '{:.1f}'.format(prob*100) + '%</span>'
                    '<span style="font-size:.8rem;font-weight:700;color:' + col + '">' +
                    ('+' if edge_sp>0 else '') + '{:.1f}'.format(edge_sp) + '%</span>'
                    '</div>'
                )
            sp_html += '</div>'
        ...
        if st.button('🔄 Nuova Partita'):
            for k in ['step','raw','banners','result_v6','h_name','a_name']+['q_home', 'q_draw', 'q_away', 'q_ov25', 'q_un25', 'q_ov35', 'q_un35', 'q_btts', 'q_bttsn', 'h_gf', 'h_ga', 'h_xg', 'h_xga', 'h_cor', 'h_crd', 'h_sot', 'h_form', 'a_gf', 'a_ga', 'a_xg', 'a_xga', 'a_cor', 'a_crd', 'a_sot', 'a_form', 'sp_home', 'sp_draw', 'sp_away', 'sp_ov25', 'sp_btts', 'motiv_h', 'motiv_a', 'days_h', 'days_a', 'pressure_h', 'pressure_a', 'ref_name', 'ref_style']:
                st.session_state.pop(k,None)
            st.session_state['step']='input'; st.rerun()
        ...
        tabs=st.tabs(['1X2 & DC','O/U & Tempi','BTTS & Multi','Gol Squadre',
                      'Speciali','Combo','H2H & Squadre','Confronto Modello','🛡️ Safety Check','💰 ROI'])
        ...
        # TAB ROI TRACKER (tab index 9)
        with tabs[9]:
            st.markdown('<div class="sec-title">💰 ROI Tracker — Storico Scommesse</div>', unsafe_allow_html=True)
            if 'roi_bets' not in st.session_state:
                st.session_state.roi_bets = []
            with st.expander('➕ Registra scommessa', expanded=True):
                rc1,rc2,rc3 = st.columns(3)
                with rc1:
                    roi_mkt   = st.text_input('Mercato', placeholder='es. 1 Casa', key='roi_mkt')
                    roi_stake = st.number_input('Puntata (€)', 0.0, 10000.0, 10.0, 1.0, key='roi_stake')
                with rc2:
                    roi_quota  = st.number_input('Quota', 1.0, 50.0, 2.0, 0.05, key='roi_quota')
                    roi_result = st.selectbox('Risultato', ['In attesa','Vinta','Persa','Void'], key='roi_res')
                with rc3:
                    roi_note = st.text_input('Note', placeholder='es. Napoli-Milan', key='roi_note')
                    roi_date = st.text_input('Data', value=str(datetime.now().date()), key='roi_date')
                if st.button('💾 Salva', key='roi_save'):
                    h_obj=R.get('h'); a_obj=R.get('a')
                    match_name=(h_obj.name+' vs '+a_obj.name) if h_obj and a_obj else roi_note
                    st.session_state.roi_bets.append({
                        'data':roi_date,'partita':match_name,'mercato':roi_mkt,
                        'stake':float(roi_stake),'quota':float(roi_quota),
                        'risultato':roi_result,'note':roi_note
                    })
                    st.success('✅ Salvata!')
            bets=st.session_state.roi_bets
            if bets:
                vinte=[b for b in bets if b['risultato']=='Vinta']
                perse=[b for b in bets if b['risultato']=='Persa']
                tot_stake=sum(b['stake'] for b in bets if b['risultato'] in ('Vinta','Persa'))
                profitto=sum(b['stake']*(b['quota']-1) for b in vinte)-sum(b['stake'] for b in perse)
                roi_pct=(profitto/tot_stake*100) if tot_stake>0 else 0.0
                win_rate=(len(vinte)/max(1,len(vinte)+len(perse)))*100
                sc1,sc2,sc3,sc4=st.columns(4)
                with sc1: st.metric('Scommesse',str(len(vinte)+len(perse)))
                with sc2: st.metric('Win Rate','{:.1f}%'.format(win_rate))
                with sc3: st.metric('Profitto','€{:+.2f}'.format(profitto))
                with sc4: st.metric('ROI','{:+.1f}%'.format(roi_pct))
                st.markdown('**Storico**')
                for b in reversed(bets):
                    col='var(--grn)' if b['risultato']=='Vinta' else ('var(--red)' if b['risultato']=='Persa' else 'var(--yel)')
                    pl=b['stake']*(b['quota']-1) if b['risultato']=='Vinta' else (-b['stake'] if b['risultato']=='Persa' else 0)
                    st.markdown(
                        '<div style="display:flex;justify-content:space-between;padding:8px 12px;'
                        'background:var(--bg2);border-radius:8px;margin-bottom:5px;font-size:.72rem">'
                        '<span>'+str(b.get('data',''))+' — '+str(b.get('partita',''))+'</span>'
                        '<span style="color:var(--t2)">'+str(b.get('mercato',''))+' @ '+'{:.2f}'.format(float(b.get('quota',0)))+'</span>'
                        '<span style="font-weight:700;color:'+col+'">'+str(b['risultato'])+' '+('+' if pl>=0 else '')+'{:.2f}'.format(pl)+'€</span>'
                        '</div>',unsafe_allow_html=True)
                if st.button('🗑️ Cancella', key='roi_clear'):
                    st.session_state.roi_bets=[]; st.rerun()
            else:
