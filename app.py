...
def main():
    ...
    with st.expander('QUOTE BOOKMAKER (opzionale ma consigliato)', expanded=False):
        ...
        with qc1:
            q_home = st.number_input('Quota 1 (Casa)',     0.0,50.0,0.0,0.05,key='qh')
            q_ov15 = st.number_input('Over 1.5',           0.0,50.0,0.0,0.05,key='qo15')
            q_ov25 = st.number_input('Over 2.5',           0.0,50.0,0.0,0.05,key='qo25')
            q_ov35 = st.number_input('Over 3.5',           0.0,50.0,0.0,0.05,key='qo35')
        with qc2:
            q_draw = st.number_input('Quota X (Pareggio)', 0.0,50.0,0.0,0.05,key='qd')
            q_un15 = st.number_input('Under 1.5',          0.0,50.0,0.0,0.05,key='qu15')
            q_un25 = st.number_input('Under 2.5',          0.0,50.0,0.0,0.05,key='qu25')
            q_un35 = st.number_input('Under 3.5',          0.0,50.0,0.0,0.05,key='qu35')
        with qc3:
            q_away  = st.number_input('Quota 2 (Trasf.)',  0.0,50.0,0.0,0.05,key='qa')
            q_bttsy = st.number_input('BTTS Si',           0.0,50.0,0.0,0.05,key='qby')
            q_bttsn = st.number_input('BTTS No',           0.0,50.0,0.0,0.05,key='qbn')
        st.markdown('**Mercati secondari**')
        sc1,sc2,sc3,sc4 = st.columns(4)
        with sc1: q_corn_ov = st.number_input('Angoli Over 9.5',  0.0,50.0,0.0,0.05,key='qcov')
        with sc2: q_corn_un = st.number_input('Angoli Under 9.5', 0.0,50.0,0.0,0.05,key='qcun')
        with sc3: q_card_ov = st.number_input('Cart. Over 3.5',   0.0,50.0,0.0,0.05,key='qkov')
        with sc4: q_card_un = st.number_input('Cart. Under 3.5',  0.0,50.0,0.0,0.05,key='qkun')
