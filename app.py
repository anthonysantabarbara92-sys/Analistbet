...
COMPETITIONS: Dict[str, List[str]] = {
    ...
    'SE Svezia': [
        ...
        'Division 2 (4o livello)', 'Svenska Cupen',
    ],
    'SCO Scozia': [
    ...
    'FI Finlandia': [
        'Veikkausliiga / Eliteserien (Div. 1)', 'Ykkonen / 1. divisjon (Div. 2)',
...
class FinanceManager:
    ...
    def analyse(cls, label, p, bk, br):
        ...
        sig  = ('[GRN] STRONG' if (val and e>0.05) else
                ('[YEL] WATCH'  if (val and e>0.02) else '[RED] SKIP'))
...
_ODDS_PROMPT = (
    ...
    '  "mg_01": null, "mg_12": null, "mg_23": null, "mg_34": null, "mg_45": null, "mg_5p": null,\n'
...
_QUALITATIVE_PROMPT = (
    ...
    'Categorie: INFORTUNI | FORMA RECENTE | METEO/CAMPO | MOTIVAZIONE | ARBITRO | '
    "TATTICA | STORICO PRECEDENTI | PRESSIONE CLASSIFICA | FATIGUE/SCHEDULE | MERCATO/RUMORS\n"
)


# ==============================================================================
#  CLASS 3 -- SMART PARSER (Claude AI)
# ==============================================================================
class SmartParser:
    """
    Claude (Anthropic) -- estrae statistiche e quote da testo grezzo.
    ...
    def _call(self, system: str, user: str) -> Dict:
        try:
            ...
            msg = client.messages.create(
                ...
                max_tokens=2048,
                system=system,
                messages=[{'role': 'user', 'content': f"TESTO INPUT:\n{user}"}]
            )
            text = msg.content[0].text.strip()
            text = re.sub(r"```json\s*|\s*```", '', text).strip()
            return {'ok': True, 'data': json.loads(text), 'raw': text}
        except json.JSONDecodeError as e:
            return {'ok': False, 'error': f"JSON parse: {e}", 'raw': text if 'text' in dir() else ''}
    ...
    def qualitative(self, context: str, summary: str) -> str:
        try:
            client = anthropic.Anthropic(api_key=self._key)
            prompt = (f"SOMMARIO MATEMATICO:\n{summary}\n\n"
                      f"RETROSCENA / CONTESTO:\n{context}")
            msg = client.messages.create(
                model='claude-opus-4-5',
                max_tokens=1024,
                system=self.QUALITATIVE_PROMPT,
...
def cards_fig(cards_range, h_avg, a_avg):
    ...
    fig.update_layout(**PL,title=_ptitle(f"CARDS  L={lam:.2f}"),height=290,showlegend=False)
    for c in [1,2]:
        fig.update_yaxes(ticksuffix='%',gridcolor='#1e1e35',row=1,col=c)
    return fig
...
def mc_fig(mc):
    ...
    fig.update_layout(**PL,title=_ptitle(f"MONTE CARLO  mu={mc['mu']:.2f} +/- sigma={mc['sig']:.2f}"),
...
# ==============================================================================
def main():
    st.set_page_config(page_title='Quantum Football v4', page_icon='[QUANT]',
    ...
    with st.sidebar:
        ...
        claude_key = st.text_input('[KEY] CLAUDE API KEY (Anthropic)', type='password', placeholder='sk-ant-...')
        st.markdown('---')
        st.markdown(
            '<div style="font-size:.6rem;color:#8888aa;line-height:1.8">'
            'Quantum Football v4.0<br>'
            'Poisson - Kelly - Monte Carlo<br>'
            'AI Cross-Check (Claude)</div>',
    ...
    st.markdown(
        ...
        '<div class="sub">Poisson - Risultati Esatti - Multigoal - AH/EH - HT/FT - Cartellini - Angoli - Goalscorer - Monte Carlo - AI Cross-Check</div>'
        '</div>',
        unsafe_allow_html=True
    )

    # -- CONFIG IN PAGINA (mobile-friendly) -----------------------------------
    with st.expander('[CFG] CONFIGURAZIONE PARTITA', expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            country = st.selectbox('Paese', options=list(COMPETITIONS.keys()), index=0)
        with c2:
            competition = st.selectbox('Competizione', options=COMPETITIONS[country])

        c3, c4 = st.columns(2)
        with c3:
            h_name = st.text_input('Squadra Casa', value='LAZIO')
        with c4:
            a_name = st.text_input('Squadra Trasferta', value='ATALANTA')

        c5, c6 = st.columns(2)
        with c5:
            bankroll = st.number_input('Bankroll (EUR)', 100.0, 1_000_000.0, 1000.0, 100.0)
        with c6:
            n_sim = st.select_slider('Monte Carlo N', [1_000, 5_000, 10_000, 50_000], 10_000)

    st.markdown(
        f'<div class="comp-badge">[CUP] {country.split()[-1]} -- {competition}</div>',
        unsafe_allow_html=True
    )
    ...
    with tabs[0]:
        # SMART SCRAPERS
        with st.expander('[AI] SMART SCRAPER -- STATISTICHE (SofaScore / FBRef / WhoScored)', expanded=False):
            ...
            if st.button('[PARSE] PARSE STATISTICHE'):
                if not claude_key: st.error('Inserisci Claude API Key (Anthropic)!')
                elif raw_stats.strip():
                    with st.spinner('Claude analizza...'):
                        r = SmartParser(claude_key).parse_stats(raw_stats)
                        if r['ok']:
        ...
        with st.expander('[AI] SMART SCRAPER -- QUOTE (Oddsportal / BetExplorer / Flashscore)', expanded=False):
            raw_odds = st.text_area('Testo quote grezzo', height=160,
                placeholder=f"Incolla quote bookmaker per {competition}...", key='ro')
            if st.button('[ODDS] PARSE QUOTE'):
                if not claude_key: st.error('Inserisci Claude API Key (Anthropic)!')
                elif raw_odds.strip():
                    with st.spinner('Claude analizza...'):
                        r = SmartParser(claude_key).parse_odds(raw_odds)
                        if r['ok']:
                            st.success('[OK] Parsing completato!')
        ...
        with c1:
            ...
            hwt  = st.slider('Recency Weight', 0.80, 1.40, float(ps.get('home_weight_recent',1.0)), 0.05, key='hwt',
                             help='1.2 = ultimi 5 match pesano 20% in piu')
        with c2:
        ...
        st.markdown('---')
        # ODDS INPUT
        st.markdown(sh_('[ODDS]', 'QUOTE BOOKMAKER'), unsafe_allow_html=True)
        ...
        # HT/FT
        st.markdown('---')
        st.markdown(sh_('[TIME]', 'HT/FT QUOTE'), unsafe_allow_html=True)
        ...
        st.markdown('---')
        st.markdown(sh_('[TARGET]', 'QUOTE RISULTATI ESATTI -- INSERIMENTO MANUALE'), unsafe_allow_html=True)
        ...
        for idx, score in enumerate(common_scores):
            ...
            val = col.number_input(f"[BALL] {score}", value=default_val,
        ...
        # Extra scores
        with st.expander('[ADD] Aggiungi risultati esatti personalizzati', expanded=False):
        ...
        # GOALSCORER
        st.markdown('---')
        st.markdown(sh_('[BALL]', 'ANYTIME GOALSCORER'), unsafe_allow_html=True)
        players_from_parse = ps.get('players', [])
        n_pl = st.number_input('No giocatori', 0, 8, max(len(players_from_parse),2), 1)
        players_input = []
        ...
        # NOTES
        st.markdown('---')
        qual_notes = st.text_area('[NEWS] Retroscena / Infortuni / Meteo / Arbitro / Rumors',
                                   height=90, key='notes')
        ...
        st.info(f"[OK] Dati salvati -- {competition} | {h_name} vs {a_name} -- passa agli altri tab.")
    ...
    with tabs[1]:
        ...
        st.markdown(grid(
            mc_('L '+hn,    f"{R['lam_h']:.4f}", 'Expected Goals Home','org'),
            mc_('L '+an,    f"{R['lam_a']:.4f}", 'Expected Goals Away','blu'),
            mc_('E[Goals]', f"{R['lam_h']+R['lam_a']:.3f}",
                f"CI 95%: {R['mc']['ci_lo']:.1f}-{R['mc']['ci_hi']:.1f}",'yel'),
            mc_('sigma Goals',  f"{R['mc']['sig']:.3f}", 'Std Dev MC','red'),
        ), unsafe_allow_html=True)

        st.markdown(sh_('[CHART]','PROBABILITA 1X2 & GOAL'), unsafe_allow_html=True)
        st.markdown(grid(
        ...
        with c2:
            ...
            st.markdown(sh_('[TARGET]','TOP EXACT SCORES -- QUICK VIEW'), unsafe_allow_html=True)
        ...
        # Shot Quality
        st.markdown(sh_('[TARGET]','SHOT QUALITY INDEX'), unsafe_allow_html=True)
    ...
    with tabs[2]:
        ...
        st.markdown(sh_('[TARGET]','RISULTATI ESATTI -- ANALISI COMPLETA'), unsafe_allow_html=True)
        ...
        # Full Table -- top 30
        st.markdown(sh_('[TABLE]','TABELLA COMPLETA -- TOP 30 RISULTATI ESATTI'), unsafe_allow_html=True)
        ...
        for s in R['top_scores']:
            ...
            rows.append({
                ...
                'STAKEEUR':    f"EUR{kl['stake']:.2f}" if kl['stake']>0 else '--',
                'STAKE%':    f"{kl['pct']:.2f}%" if kl['stake']>0 else '--',
                'EVEUR':       f"EUR{kl['ev']:.2f}" if kl['stake']>0 else '--',
                'VALUE':     '[OK]' if val else ('[--]' if bk>1 else '--'),
            })
        ...
        # Value hits detail
        if val_scores:
            st.markdown(sh_('[GEM]','VALUE SCORES -- DETTAGLIO KELLY'), unsafe_allow_html=True)
                  ...
                  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                    ...
                    <div style="display:flex;gap:20px;flex-wrap:wrap;">
                      ...
                      <div style="text-align:center;">
                        ...
                        <div style="font-size:.9rem;font-weight:600;color:#e8e8f0;">EUR{kl['stake']:.2f}</div>
                      </div>
                      <div style="text-align:center;">
                        <div class="s-label">EV</div>
                        <div style="font-size:.9rem;font-weight:600;color:#00ff88;">EUR{kl['ev']:.2f}</div>
                      </div>
                    </div>
                  </div>
            ...
            st.info('[i] Nessuna value bet su risultati esatti. Inserisci le quote nel Tab INPUT -> Risultati Esatti.')

        # Category breakdown
        st.markdown('---')
        st.markdown(sh_('[CHART]','BREAKDOWN PER CATEGORIA'), unsafe_allow_html=True)
    ...
    with tabs[3]:
        ...
        mg_rows=[{'RANGE':k,'PROB%':f"{p*100:.2f}%",'FAIR':f"{fm.fair(p):.2f}",
                  ...
                  'VALUE':'[OK]' if (mg_map.get(k,0) and mg_map[k]>fm.fair(p)) else '[NO]'}
                 for k,p in R['mg'].items()]
        st.dataframe(pd.DataFrame(mg_rows),use_container_width=True,hide_index=True)

        st.markdown('---')
        # ASIAN HANDICAP
        st.markdown(sh_('[SPEC]','ASIAN HANDICAP'), unsafe_allow_html=True)
        ...
        for h in R['ah_lines']:
            ...
            ah_rows.append({'HC':f"{h:+.1f}",'HOME%':f"{ph*100:.1f}%",
                            ...
                            'STAKE':f"EUR{kl['stake']:.0f}" if kl['stake']>0 else '--'})
        st.dataframe(pd.DataFrame(ah_rows),use_container_width=True,hide_index=True)

        st.markdown('---')
        # EH
        st.markdown(sh_('[EH]','EUROPEAN HANDICAP'), unsafe_allow_html=True)
        ...
        for combo,prob in R['htft'].items():
            ...
            htft_rows.append({'HT/FT':combo,'PROB%':f"{prob*100:.2f}%",'FAIR':f"{fair:.2f}",
                               ...
                               'STAKE':f"EUR{kl['stake']:.0f}" if kl['stake']>0 else '--',
                               'VALUE':'[OK]' if (bk and bk>fair) else '[NO]'})
        st.dataframe(pd.DataFrame(htft_rows),use_container_width=True,hide_index=True)

        st.markdown('---')
        # CARTELLINI
        st.markdown(sh_('[CARD]','CARTELLINI'), unsafe_allow_html=True)
        cr=R['cards']
        st.markdown(grid(
    ...
    with tabs[4]:
        ...
        st.markdown(sh_('[VALUE]','VALUE BET DETECTOR'), unsafe_allow_html=True)
        st.markdown(grid(
            mc_('VALUE BETS', str(len(val_mkt)), f"su {len(markets)} mercati",'grn'),
            mc_('STAKE TOT',  f"EUR{t_stake:.0f}", f"{t_stake/br*100:.1f}% bankroll",'org'),
            mc_('EV TOTALE',  f"EUR{t_ev:.2f}",    'Expected Value aggregato','grn' if t_ev>0 else 'red'),
            mc_('BEST EDGE',  f"{b_edge:.1f}%",   'Massimo vantaggio','yel'),
        ), unsafe_allow_html=True)

        st.markdown(sh_('[TABLE]','FULL MARKET TABLE'), unsafe_allow_html=True)
        ...
        if val_mkt:
            st.markdown(sh_('[SPEC]','TOP VALUE BETS -- KELLY DETAIL'), unsafe_allow_html=True)
                  ...
                  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                    ...
                    <div style="display:flex;gap:16px;flex-wrap:wrap;">
                      ...
                      <div style="text-align:center;"><div style="font-size:.57rem;color:#8888aa;">STAKE</div>
                        <div style="font-size:1.05rem;font-weight:700;color:#e8e8f0;">EUR{m['stake']:.2f}</div></div>
                      <div style="text-align:center;"><div style="font-size:.57rem;color:#8888aa;">EV</div>
                        <div style="font-size:1.05rem;font-weight:700;color:{'#00ff88' if m['ev']>0 else '#ff3355'};">EUR{m['ev']:.2f}</div></div>
                    </div>
                  </div>
    ...
    with tabs[5]:
        ...
        st.markdown(sh_('[MC]',f"MONTE CARLO -- {rp['n_sim']:,} SIM"), unsafe_allow_html=True)
        st.markdown(grid(
        ...
        st.markdown(grid(
            mc_('mu GOALS',  f"{mc['mu']:.3f}",f"sigma={mc['sig']:.3f}",'yel'),
        ...
        with c2:
            ...
            stab = '[GRN] ALTA' if d<1.5 else ('[YEL] MEDIA' if d<3.5 else '[RED] BASSA')
            st.markdown(f"""
            <div style="margin-top:12px;padding:14px 16px;background:#12121e;
                        border:1px solid #1e1e35;border-radius:6px;">
              <div style="font-size:.57rem;color:#8888aa;letter-spacing:.15em;">STABILITA</div>
              <div style="font-size:1.1rem;font-weight:700;color:#e8e8f0;margin-top:4px;">{stab}</div>
              <div style="font-size:.63rem;color:#8888aa;margin-top:3px;">Delta Home: {d:.2f} pp</div>
            </div>""",unsafe_allow_html=True)

        # Variance Cloud
        st.markdown(sh_('[HEAT]','NUVOLA DI VARIANZA'), unsafe_allow_html=True)
    ...
    with tabs[6]:
        ...
        vb_txt = "\n".join(
            [f"  - {m['market']}: Edge {m['edge%']:+.2f}%, Stake EUR{m['stake']:.2f}"
             for m in val_mkt]
...
BTTS:  {R['bttsy']*100:.1f}%   LH={R['lam_h']:.3f}  LA={R['lam_a']:.3f}
MOST LIKELY SCORE: {R['top_scores'][0]['score']} ({R['top_scores'][0]['prob_pct']:.3f}%)
MC:    mu={mc['mu']:.2f} +/- sigma={mc['sig']:.2f}  CI=[{mc['ci_lo']:.1f}-{mc['ci_hi']:.1f}]
...
{summary}
        ...
        if st.button('[AI] AVVIA AI CROSS-CHECK'):
            if not claude_key:
                st.error('[WARN] Inserisci Claude API Key nella sidebar!')
            else:
                ctx = qual.strip() or 'Nessun contesto qualitativo fornito.'
                with st.spinner('Claude analizza retroscena...'):
                    try:
                        ai_txt = SmartParser(claude_key).qualitative(ctx, summary)
                        pos = ai_txt.lower().count('positivo')
                        neg = ai_txt.lower().count('negativo')
                        vc  = 'pos' if pos >= neg else 'neg'
                        vt  = '[GRN] SEGNALE CONFERMATO' if vc=='pos' else '[RED] SEGNALE INDEBOLITO'
...
{ai_txt}
        ...
        # DECISION BOARD
        st.markdown('---')
        st.markdown(sh_('[TABLE]','DECISION BOARD -- TOP BETS'), unsafe_allow_html=True)
        all_val = sorted(val_mkt + [
            {'market':f"ES {s['score']}",
             ...
             'prob%': s['prob_pct'],
             'fair':  s['fair_odd'],
             'signal':'[GRN] STRONG' if FinanceManager.edge(s['prob'],rp['odds_dict']['exact_scores'].get(s['score'],0))>0.05 else '[YEL] WATCH',
             'value': True}
        ...
        if all_val:
                ...
                <div class="market-row">
                  <div style="display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <div style="display:flex;gap:14px;">
                      ...
                      <span style="color:#e8e8f0;">EUR{m['stake']:.2f}</span>
                      <span style="color:{'#00ff88' if m['ev']>0 else '#ff3355'};">EV EUR{m['ev']:.2f}</span>
                    </div>
                  </div>
            ...
            st.markdown("""
            <div class="vbox neg">
              <strong style="color:#ff3355;">[STOP] NESSUNA BET CONSIGLIATA</strong><br>
        ...
        <div style="margin-top:20px;padding:11px 15px;background:#0f0f1a;border:1px solid #1e1e35;
          ...
          [WARN] DISCLAIMER: Strumento educativo e di ricerca quantitativa.
