...
def mc_fig(mc):
    ...
    fig.update_layout(**PL,title=_ptitle(f"MONTE CARLO  µ={mc['mu']:.2f} +/- σ={mc['sig']:.2f}"),
    for c in [1,2]:
...
def main():
 ...
 '+an,    f"{R['lam_a']:.4f}", 'Expected Goals Away','blu'),
            ...
            mc_('σ
 Goals',  f"{R['mc']['sig']:.3f}", 'Std Dev MC','red'),
        ), unsafe_allow_html=True)
        st.markdown(sh_('[CHART]','PROBABILITÀ 1X2 & GOAL'), unsafe_allow_html=True)
        st.markdown(grid(
    ...
    with tabs[3]:
        ...
        st.markdown(grid(
            mc_('λ
 CARDS', f"{cr['lam']:.2f}",'Totale atteso','yel'),
    ...
    with tabs[5]:
        ...
        st.markdown(grid(
            mc_('µ GOALS',  f"{mc['mu']:.3f}",f"σ={mc['sig']:.3f}",'yel'),
...
BTTS:  {R['bttsy']*100:.1f}%   
λH={R['lam_h']:.3f}  λA={R['lam_a']:.3f}
MOST LIKELY SCORE: {R['top_scores'][0]['score']} ({R['top_scores'][0]['prob_pct']:.3f}%)
MC:    µ={mc['mu']:.2f} +/- σ={mc['sig']:.2f}  CI=[{mc['ci_lo']:.1f}-{mc['ci_hi']:.1f}]
